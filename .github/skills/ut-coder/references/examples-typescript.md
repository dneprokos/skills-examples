...
Only mock types that represent **behavior** — interface-shaped objects with `vi.fn()` methods. Use real instances for everything else.

```typescript
// CORRECT — real object literal for DTO / value object
const address: Address = { street: '123 Main St', city: 'Springfield', zip: '62701' };
const request: CreateOrderRequest = { customerId: 42, address };

// CORRECT — mock for behavioral dependency (interface-shaped)
...
// WRONG — never mock a plain data object
const address = { street: vi.fn() } as any; // ❌  no reason to mock static data
```
...
|---|---|---|
| `UserRepository` (interface with methods) | Behavioral interface | `mock` with `vi.fn()` |
| `EmailService` (interface) | Behavioral interface | `mock` with `vi.fn()` |
| `Address` (plain object / type) | Value Object | `real` object literal |
| `Money` (immutable class) | Value Object | `real` instance |
| `CreateOrderRequest` (interface / type) | DTO | `real` object literal |
| `string`, `number`, `boolean` | Primitive | `real` value |

## Constructor Null Guard Tests (TypeScript)

For each constructor parameter that is a behavioral dependency, generate a null/undefined guard test.

TypeScript doesn't enforce null checks at runtime unless the class explicitly validates. Test explicit guards:

```typescript
// Given: constructor(private readonly repository: UserRepository, private readonly emailService: EmailService)

it.each([
  { label: 'null repository',    repo: null,      email: mockEmailService },
  { label: 'undefined repository', repo: undefined, email: mockEmailService },
  { label: 'null emailService',  repo: mockRepo,   email: null },
])('should throw TypeError when $label is provided', ({ repo, email }) => {
  expect(() => new OrderService(repo as any, email as any)).toThrow(TypeError);
});
```

If the class uses an explicit guard:

```typescript
if (!repository) throw new Error('repository is required');
```

…then assert `Error` or a custom error class and check the message:

```typescript
expect(() => new OrderService(null as any, emailService))
  .toThrow('repository is required');
```

## Non-Deterministic Abstractions (TypeScript)

When the source calls `Date.now()`, `new Date()`, `crypto.randomUUID()`, or `Math.random()`, the Analyst will flag these. Inject function-typed dependencies.

### Time abstraction — `DateProvider` function type

```typescript
// Type definition (add to your source)
export type DateProvider = () => Date;

// Source: inject DateProvider with a default
export class OrderService {
  constructor(
    private readonly repository: UserRepository,
    private readonly getDate: DateProvider = () => new Date(),
  ) {}

  placeOrder(request: CreateOrderRequest): Order {
...
it('should set createdAt to the current date', () => {
  const result = sut.placeOrder(validRequest);
  expect(result.createdAt).toEqual(FIXED_DATE);
});
```

### ID abstraction — `IdGenerator` function type
...
const sut = new OrderService(mockRepository, () => FIXED_ID);
```
